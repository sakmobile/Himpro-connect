...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `getvisitformophalert` AS select (select `vv`.`codehos` from `hosdata`.`confighos` `vv` limit 1) AS `hospcode`,replace(`p`.`cardid`,'-','') AS `cid`,`o`.`hn` AS `hn`,`o`.`docno` AS `vn`,(case when (`room`.`roomname` like '%ER%') then 'OPD' else 'ER' end) AS `department_type`,lpad(trim(`c`.`stdcode`),2,'0') AS `department_code`,coalesce(`ch`.`clinicname`,convert(`c`.`NAME` using utf8mb4)) AS `department_name`,`o`.`regdate` AS `date_service`,time_format(`o`.`timereg`,'%H:%i') AS `time_service` from (((((`opd`.`opd` `o` join `pt`.`pt` `p` on((`p`.`hn` = `o`.`hn`))) left join `hos`.`clinic` `c` on((`c`.`code` = `o`.`clinic`))) left join `his_connect`.`clinic_hdc` `ch` on((`ch`.`cliniccode` = convert(lpad(trim(`c`.`stdcode`),2,'0') using utf8mb4)))) left join `ipd`.`ipd` `ip` on(((`ip`.`hn` = `o`.`hn`) and (`ip`.`dateadm` = `o`.`regdate`) and (left(`ip`.`an`,1) <> 'X') and (`ip`.`datedsc` = '0000-00-00')))) left join `hos`.`roomno` `room` on((`room`.`roomcode` = `o`.`doctorroom`))) where ((`o`.`regdate` = curdate()) and (`o`.`result` = 'RST1') and isnull(`ip`.`an`) and (`p`.`ptnation` = '99')) group by `o`.`regdate`,`o`.`hn`,`o`.`frequency` order by `o`.`regdate`,`o`.`timereg`;
